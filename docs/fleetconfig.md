...
- `agents` - embed-only roster block. Exactly one in embedded sources.
  ...
  - `roles` - optional per-role capability roster. Each `role <name>` names its guardfile set with repeated `guardfile "..."` children and may add sparse per-agent launch-knob overlays.
