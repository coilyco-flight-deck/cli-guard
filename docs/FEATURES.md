...
- **hook** / **hookcfg** - PreToolUse engine expanding `repocfg.Security` into the guard registry and settings installer.
...
- **guarded rollback** - `compensate` + `canary` health-window rollback over a step seam. See [specverb-rollback.md](specverb-rollback.md).
