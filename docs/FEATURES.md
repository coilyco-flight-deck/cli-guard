...
- **mcporter** - Pre-exec preflight for the mcporter tool. Scans `~/.mcporter/mcporter.json` for `${VAR}` references and resolves them via a consumer-supplied `SecretResolver`, injecting the values as env vars on the child process only. Backend-agnostic; `WithTTLCache` provides on-disk caching mechanics with a consumer-owned persist path. Wired into the passthrough via `WithSecretResolver`.
- **dispatch** - Fire `claude` against a real open GitHub issue, in the matching local checkout, headless (detached `claude -p`) or interactive (new Warp tab). The security claim is generic - the prompt comes from a real open issue inside an allowed org, never free text - and host-specific bits (org, workspace layout, runner, verb pipeline) inject through `dispatch.Config`. Carries a `reap` verb for merged worktrees. Build a `Dispatcher` with `New`, hang its `Command` off the host tree.
...
- **config** - Layered-config primitives: `~/.coily` and `./.coily` path helpers, `ExpandHome`, audit-slug derivation from `git remote get-url origin`, the `Audit` rotation-knobs struct, and a generic `OverlayFile[T]` helper that consumers call once per layer (global, then local) to assemble their own schema. The consumer owns the schema; cli-guard does not.
