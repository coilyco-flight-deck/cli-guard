...
A consumer's per-area reference doc is regenerated on every build (`kdl-specs gen`/`build`), so a hand-added back-link at the bottom is wiped on the next regeneration. Emitting the footer from a guardfile node makes the back-link **generated** too: it survives every regeneration because it is re-emitted from the source. This is the enabler for [ward#309](https://forgejo.coilysiren.me/coilyco-flight-deck/ward/issues/309), whose generated `docs/ward-kdl/*.guardfile.md` reference docs could not point back to their `docs/ward-kdl.md` hub without it.
