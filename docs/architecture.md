...
The three surfaces express **permissions**; **config validation does not**, so it lives in the core (`pkg/`), never as a fourth guarded surface. `pkg/fleetconfig` types and validates the KDL fleet config - which agents a fleet runs and how each launches - and its vocabulary structurally cannot express a grant: no `mount`, no `exec`, no `can run`. The embedded roster hangs off `agents`, and role bindings repeat singular `guardfile` nodes. That package boundary **is** the config/permission partition. Keeping config in core is what keeps the three-surface least-privilege identity legible: `cli/`, `http/`, `mcp/` stay the whole permission story, and a reader never has to wonder whether a config file is also a policy file. This is the config-placement doctrine's landing in cli-guard, generalizing the downward-only arrow above. See [fleetconfig.md](fleetconfig.md).
