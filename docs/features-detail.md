...
- **sudo** - Policy-free plumbing for driving interactive sudo over any stdin-piping transport without carrying a password at rest or leaking it through argv. /dev/tty prompt, in-place buffer wipe, stderr sentinel match for `sudo -n` denial.
