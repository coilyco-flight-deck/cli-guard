...
Per-host lockdown profile registry for coily, embedded as `cli/profiles/default.yaml` and copied into `~/.coily/coily.yaml` by `coily lockdown init-config`.

Each profile is a self-contained tier-per-axis coordinate. There is no inheritance: every profile lists all four axes, every time, and the loader rejects partial entries on purpose (coilysiren/coily#150). With no override file present, coily resolves every profile to the strictest tier on every axis (the headless-default) and refuses anything not explicitly allowed. The override file is how the operator opts into looser tiers, per axis, on disk.
