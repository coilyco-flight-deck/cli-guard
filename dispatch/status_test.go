...
func TestStatus_ByRef_FiltersToRepoAndNumber(t *testing.T) {
	...
	writeFakeDispatch(t, root, "agentic-os-kai", 5, "20260301-090000", "other repo\n", nil)
	// Two logs for coily#330: pick the newer one.
	writeFakeDispatch(t, root, "coily", 330, "20260101-100000", "older 330\n", nil)
	target := writeFakeDispatch(t, root, "coily", 330, "20260201-100000",
		"newer 330\nmore newer\n", &dispatchMeta{
			PID: 99999, StartedAt: time.Date(2026, 2, 1, 10, 0, 0, 0, time.UTC), Ref: "coilysiren/coily#330",
		})

	out, err := runStatusCmd(t, d, []string{"coilysiren/coily#330"})
	if err != nil {
		t.Fatalf("status: %v", err)
	}
	if !strings.Contains(out, target) {
		t.Errorf("expected newer coily#330 log %q in output, got:\n%s", target, out)
	}
	if strings.Contains(out, "agentic-os-kai") {
...
func TestStatus_ByPID_FindsMatchingSidecar(t *testing.T) {
	...
	writeFakeDispatch(t, root, "coily", 1, "20260101-100000", "one\n", &dispatchMeta{PID: 111})
	target := writeFakeDispatch(t, root, "agentic-os-kai", 2, "20260102-100000", "two\n", &dispatchMeta{PID: 222})
	writeFakeDispatch(t, root, "coily", 3, "20260103-100000", "three\n", &dispatchMeta{PID: 333})
...
func TestStatus_PIDNotFound_Errors(t *testing.T) {
	...
	root := t.TempDir()
	d.cfg.LogRoot = func() (string, error) { return root, nil }
	writeFakeDispatch(t, root, "coily", 1, "20260101-100000", "x\n", &dispatchMeta{PID: 1})
...
func TestStatus_RejectsPidPlusRef(t *testing.T) {
	...
	_, err := runStatusCmd(t, d, []string{"--pid", "1", "coilysiren/coily#1"})
	if err == nil {
...
func TestStatus_ExitedShowsDuration(t *testing.T) {
	...
	logPath := writeFakeDispatch(t, root, "coily", 42, "20260101-100000", "done\n", &dispatchMeta{
		PID:       1, // pid 1 is init / launchd; signal-0 returns EPERM, treated as not-running
		StartedAt: time.Date(2026, 1, 1, 10, 0, 0, 0, time.UTC),
		Ref:       "coilysiren/coily#42",
	})
...
func TestWriteAndReadDispatchMeta(t *testing.T) {
	...
	m := dispatchMeta{
		...
		Ref:       "coilysiren/coily#7",
		URL:       "https://github.com/coilysiren/coily/issues/7",
	}
