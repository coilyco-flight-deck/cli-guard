...
func TestDescribeMarkdown(t *testing.T) {
	...
	// No doc-link declared -> no See also footer.
	if strings.Contains(md, "## See also") {
		t.Errorf("unexpected See also footer without a doc-link:\n%s", md)
	}
}

func TestDocLinkParsesAndRendersFooter(t *testing.T) {
	src := `wrap ward-kdl docker {
		exec docker
		can run ps
	...
	if len(gf.DocLinks) != 2 {
		t.Fatalf("want 2 doc-links, got %d", len(gf.DocLinks))
	}
	if gf.DocLinks[1].Text != "ward-kdl-surface.md" { // text defaults to href
		t.Errorf("text default: got %q want href", gf.DocLinks[1].Text)
	}
	md := Describe(gf).Markdown()
	for _, want := range []string{
		"\n## See also\n\n",
	...
	// The footer trails the verb stanzas.
	if strings.Index(md, "## See also") < strings.Index(md, "## ward-kdl docker ps") {
		t.Errorf("See also footer must come after the verb stanzas:\n%s", md)
	}
}

func TestDocLinkOnInspectListFooter(t *testing.T) {
	src := `wrap ward-kdl pkg brew {
		allow brew
		doc-link "../ward-kdl-surface.md" "ward-kdl-surface.md"
	}`
	gf, err := Parse([]byte(src))
	if err != nil {
		t.Fatalf("parse: %v", err)
	...
	md := Describe(gf).Markdown()
	if !strings.Contains(md, "- [ward-kdl-surface.md](../ward-kdl-surface.md)\n") {
		t.Errorf("inspect-list surface dropped the doc-link footer:\n%s", md)
	}
}

func TestDocLinkRejectsEmptyAndBlock(t *testing.T) {
	for _, src := range []string{
		...
		if _, err := Parse([]byte(src)); err == nil {
			t.Errorf("expected parse error (fail-closed) for:\n%s", src)
		}
	}
}
