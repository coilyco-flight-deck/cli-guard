...
// exec (wrapped binary/execverb). See docs/kdl-specs.md.
const (
	TransportSpec = "spec"
	TransportExec = "exec"
)
...
type Params struct {
	...
	SpecURL       string // upstream Swagger URL; the `kdl-specs lock` source and bootstrap fallback
...
func Plan(gf *guardfile.Guardfile, guardfileName string) (Params, error) {
	...
	return Params{
		...
		// into one binary get distinct overrides (see docs/kdl-specs.md).
...
// names its env injections use; no upstream spec. See kdl-specs.md.
...
// distinct tracked name; the pruned lock is always JSON). See kdl-specs.md.
...
var mainTemplate = template.Must(template.New("main").Parse(`// Code generated by kdl-specs; DO NOT EDIT.
// Merged from each consumer guardfile. Regenerate with 'kdl-specs gen' (or 'run').
package main
...
// lock. Refresh both with 'kdl-specs lock'.
{{range $i, $m := .Mounts}}
...
func main() {
	app := &cli.Command{Name: "{{.Binary}}", Usage: "guarded verbs generated by kdl-specs", Version: Version}
...
// bootstrap used only before the first 'kdl-specs lock'.
func resolveSpec(specLock []byte, specURL, specEnv string) ([]byte, error) {
	...
	fmt.Fprintf(os.Stderr, "{{.Binary}}: no embedded spec lock; fetching %s (run 'kdl-specs lock')\n", specURL)
